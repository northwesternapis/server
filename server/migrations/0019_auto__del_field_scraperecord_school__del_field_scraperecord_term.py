...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'ScrapeRecord.school'
        db.delete_column(u'server_scraperecord', 'school_id')

        # Deleting field 'ScrapeRecord.term'
        db.delete_column(u'server_scraperecord', 'term_id')


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'ScrapeRecord.school'
        raise RuntimeError("Cannot reverse this migration. 'ScrapeRecord.school' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'ScrapeRecord.school'
        db.add_column(u'server_scraperecord', 'school',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['server.School']),
                      keep_default=False)


        # User chose to not deal with backwards NULL issues for 'ScrapeRecord.term'
        raise RuntimeError("Cannot reverse this migration. 'ScrapeRecord.term' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'ScrapeRecord.term'
        db.add_column(u'server_scraperecord', 'term',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['server.Term']),
                      keep_default=False)
